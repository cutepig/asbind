...
You should create a Context object like this :

	ASBind::Context ctx( asContext, asModule );

Then lets say you have a 'main' function in the script like this

	void main() { ... }

You create a function pointer object and fetch it from the context :

	ASBind::FunctionPtr<void()> mainPtr = ctx.func( "main", mainPtr );

Note that the FunctionPtr has a matching function prototype as a template argument
and the instance is also passed as a reference to ctx.func. This is only a dummy parameter
...
You can fetch FunctionPtr's directly with the id (if you know it)

	ASBind::FunctionPtr<void()> mainPtr = ASBind::CreateFunctionPtr( id, asContext, mainPtr );

Or by name (note the additional module parameter, needed to resolve the name to id)

	ASBind::FunctionPtr<void()> mainPtr = ASBind::CreateFunctionPtr( "main", asContext, asModule, mainPtr );
...
#ifndef ASBIND_THROW
	#include <stdexcept>
	#define AS_BIND_THROW(a)	throw ASBind::Exception(a)
#endif
...
struct FunctionPtrBase {
	...
	FunctionPtrBase( int _id, asIScriptContext *_ctx )
		: id( _id ), ctx( _ctx ) {}
	...
	void precall( void ) { if( id ) ctx->Prepare( id ); }
	void call( void ) {
		if( ctx ) {
			int r = ctx->Execute();
			if( r != asEXECUTION_FINISHED ) {
				Com_Printf("ASBind::FunctionPtrBase: Execute failed %d\n", r);
				// throw std::string( "FunctionPtrBase::call Execute failed", r );
			}
		}
	}
};
...
struct FunctionPtr : FunctionPtrBase {
	FunctionPtr( int _id=0, asIScriptContext *_ctx=0 ) : FunctionPtrBase( _id, _ctx ) {}
	R operator()( void )
	{
		ctassert<false>();
...
template<typename F>
FunctionPtr<F> CreateFunctionPtr( int id, asIScriptContext *ctx, FunctionPtr<F> &f )
{
	return FunctionPtr<F>( id, ctx );
}

template<typename F>
FunctionPtr<F> CreateFunctionPtr( const char *name, asIScriptContext *ctx, asIScriptModule *mod, FunctionPtr<F> &f )
{
	// get the id
	int id = mod->GetFunctionIdByDecl( FunctionString<typename FunctionPtr<F>::func_type>( name ).c_str() );
	return CreateFunctionPtr<F>( id, ctx, f );
}
...
public:
	// constructors
	Class( asIScriptEngine *_engine ) : engine( _engine )
	{
		name = TypeString<T>();
		registerSelf();
	}

	Class( asIScriptEngine *_engine, const char *_name )
	...
	Class( asIScriptEngine *_engine, const char *_name, int _id )
		: engine( _engine ), name( _name ), id( _id )
	{}
	...
	Class & constmethod( F f, const char *fname, bool obj_first )
	{
		std::string constname = ( obj_first ?
			FunctionString<StripThisProxy<F>::func_of>( fname ) :
			FunctionString<StripThisProxy<F>::func_ol>( fname ) ) + " const";
	...
	{
		std::string funcname = obj_first ?
			FunctionString<StripThisProxy<F>::func_of>( "f" ) :
			FunctionString<StripThisProxy<F>::func_ol>( "f" );
	...
	Class & cast( F f )
	{
		std::string constname = MethodString<F>( "f" ) + " const";
		int _id = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_VALUE_CAST,
			constname.c_str(), asSMethodPtr<sizeof( f )>::Convert( f ), asCALL_THISCALL );
		if( _id < 0 )
	...
	// input method
	template<typename F>
	Class & cast( F f, bool obj_first )
	{
		std::string constname = ( obj_first ?
			FunctionString<StripThisProxy<F>::func_of>( "f" ) :
			FunctionString<StripThisProxy<F>::func_ol>( "f" ) ) + " const";
...
public:
	// constructors
	Global( asIScriptEngine *_engine ) : engine( _engine )
	{}
...
// asbind_context.h
// use Context to fetch global function objects
...
public:
	Context( asIScriptContext *_ctx, asIScriptModule *_mod )
		: context( _ctx ), module( _mod ) // , engine( _ctx->GetEngine() )
	{}

	template<typename F>
	FunctionPtr<F> func( const char *name, FunctionPtr<F> &f )
	{
		// THIS BREAKS : FunctionString<F>( name );
		// so we take the long route
		// FunctionString<FunctionPtr<F>::func_type>( name )
		std::string funcname = FunctionString<FunctionPtr<F>::func_type>( name );
		int funcId = module->GetFunctionIdByDecl( funcname.c_str() );
		if( funcId < 0 )
			throw Exception( va( "ASBind::Context::func (%s) GetFunctionIdByDecl failed %d", funcname.c_str(), funcId ) );

		return CreateFunctionPtr<F>( funcId, context, f );
	}
};
